...
void process_packet(u_char *args,const struct pcap_pkthdr *header,const u_char * packet)
{
	...
	else
	{
		...
		{
			strcpy(Packet_Num[count].src_ip,inet_ntoa(ip_packet->ip_src));
			strcpy(Packet_Num[count].dst_ip,inet_ntoa(ip_packet->ip_dst));
			Packet_Num[count].num=count;
		...
		if(size_payload>1)
		{
			...
			while(1)
			{
				...
				if(flag<Get_flag)
				{
					...
					{
						//*Temp=*Temp+4;
						strcpy(Packet_Num[count].Get_Header,Temp);
						//Packet_Num[count].Get_Header[]
						memset(Temp,0,800);
						Get_i=0;
					...
					{
						//*Temp=*Temp+5;
						strcpy(Packet_Num[count].Host,Temp);
						memset(Temp,0,800);
						Get_i=0;
						//break;
					}
					...
					{
						//*Temp=*Temp+11;
						strcpy(Packet_Num[count].User_Agent,Temp);
						memset(Temp,0,800);
						Get_i=0;
					}
					else
			...
			char Ful_Te1[800];
			int Ful_i=4;
			//*Ful_Temp=*Packet_Num[count].Get_Header+4;
			while(Packet_Num[count].Get_Header[Ful_i])
			{
				
				if(Packet_Num[count].Get_Header[Ful_i]==' ')
					break;
				else
				{

					Ful_Temp[Ful_i-4]=Packet_Num[count].Get_Header[Ful_i];
					Ful_i++;
				}
			...
			Ful_Temp[Ful_i-4]='\0';
			printf("%s\n",Ful_Temp);
			for(Ful_i=6;Ful_i<strlen(Packet_Num[count].Host)+6;Ful_i++)
			{
				printf("222\n");
				Ful_Te1[Ful_i-6]=Packet_Num[count].Host[Ful_i];				
			}
			Ful_Te1[Ful_i-6]='\0';
			printf("%s\n",Ful_Te1);
			strcat(Ful_Te1,Ful_Temp);

			printf("Num:%d\n",Packet_Num[count].num);
			printf("dst_ip:%s\n",Packet_Num[count].dst_ip);
			printf("src_ip:%s\n",Packet_Num[count].src_ip);
			printf("Get_Header:%s\n",Packet_Num[count].Get_Header);
			printf("User_Agent:%s\n",Packet_Num[count].User_Agent);
			printf("Host:%s\n",Packet_Num[count].Host);
			printf("Full_Url:%s\n",Ful_Te1);
			count++;		
...
int main(int argc, char const *argv[])
{
	...
	if((r==pcap_loop(handle,Packet_Num,process_packet,NULL))<0)
	{
